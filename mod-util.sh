...
BBok=true
if [ "$_bbname" == "" ]; then
  _bbname="BusyBox is optional, but is needed in some case e.g. (using pandora on stock oem roms etc.)"
  BBok=false
fi
...
test_connection() {
  ...
  if /system/bin/ping -q -c 1 -W 1 google.com >/dev/null 2>&1; then
    ...
    false
  fi & e_spinner " Checking connection"
  ) && echo " - ${G}OK${N}" || { echo " - ${R}Error${N}"; false; }
  sleep 1
}
...
upload_logs() {
  $BBok && {
    ...
    O_Log: $oldlogUp
    Log:   $logUp" | nc termbin.com 9999 & e_spinner "Testing internet connection"
	) && echo " - OK" || { echo " - Error"; false; }
	e_spinner "Uploading logs"
